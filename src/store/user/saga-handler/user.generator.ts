...
import {
    ...
    getNumberOfMessagesNotSeeSuccess, getNumberOfMessagesNotSeeFailure, fetchUsersSuccess, fetchUsersFailure, addRemoveAdminSuccess, addRemoveAdminFailure
} from "../slice";
 ...
 *
 */
export function* blockedUnblockeUserHandler(data: any): Generator<any, any, any> {
    try {
        const result = yield invokeWS({
            url: `${apiUrl}admin/blocked-user/${data.payload.userId}`,
            method: MethodHttp.get,
        }, {lockUnblock: data.payload.blockUnblock})
        yield put(fetchUsersSuccess(result.data));
    } catch (e) {
        console.error(e);
        yield put(fetchUsersFailure(e));
